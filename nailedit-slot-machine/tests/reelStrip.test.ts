import { CELL, ROW_COUNT, SPIN, SymbolId } from '../src/config';
...
test('a fresh strip exposes exactly the visible rows', () => {
    const strip = new ReelStrip(fillerSequence());
    assertEqual(strip.visibleSymbols.length, ROW_COUNT, 'visible symbol count');
});
...
test('a stop sequence lands exactly on the requested column', () => {
    const strip = landAfterSpin(column('high1', 'low2', 'high3'), 12.37, SPIN.maxSpeed);
...
test('landing is exact from any starting fraction', () => {
    for (let i = 0; i < 100; i++) {
        ...
        const strip = landAfterSpin(column('low4', 'high2', 'low1'), startCells, SPIN.maxSpeed);
...
test('landing is exact across a range of stop speeds', () => {
    for (let speed = SPIN.maxSpeed * 0.5; speed <= SPIN.maxSpeed * 1.5; speed += 137) {
...
test('the strip comes to rest aligned to a whole cell', () => {
    const strip = landAfterSpin(column('high1', 'high1', 'high1'), 9.81, SPIN.maxSpeed);
...
test('scrolling forward keeps the window full', () => {
    ...
    assertEqual(strip.visibleSymbols.length, ROW_COUNT, 'visible symbol count while spinning');
    assert(
